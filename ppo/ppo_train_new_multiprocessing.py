...
from lib.multiproc import multiProcess

NUM_ENVS            = 8                 # number of environments
ENV_ID              = "Pendulum-v0"
HIDDEN_SIZE         = 256               # neurons in hidden layer of NN
LEARNING_RATE       = 1e-4
GAMMA               = 0.99              # discount factor to calculate returns
GAE_LAMBDA          = 0.95              # smoothing factor
PPO_EPSILON         = 0.2               # to clip ratio between new and old policy
CRITIC_DISCOUNT     = 0.5               # to decrease critic loss, since it is higher that actor loss
ENTROPY_BETA        = 0.001             # amount of importance to entropy bonus
PPO_STEPS           = 256               # num steps in environments
MINI_BATCH_SIZE     = 64                # num of samples selected from total amount of stored data
PPO_EPOCHS          = 10                # number of passes over training data
TEST_EPOCHS         = 10                # how often we run test to test performance
NUM_TESTS           = 10                # num of test to average reward over
TARGET_REWARD       = 2500              # when this reward is reached training training stops
...
if __name__ == "__main__":
    ...
    envs = [gym.make(ENV_ID) for i in range(NUM_ENVS)]                              #XXXXXXXX#
    envs = multiProcess(envs)                                                       #XXXXXXXX#
    env = gym.make(ENV_ID)
